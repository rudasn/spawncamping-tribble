...
angular.module('myApp.controllers', [])
  ...
  .controller('ChatCtrl', ['$scope', 'syncData', function($scope, syncData) {
      ...
      var offset = new Date().valueOf();
      window.msg = function() {
         $scope.newMessage = Math.floor((new Date().valueOf() - offset) / 1000);
         $scope.addMessage();
      };
      window.START = function(m) {
         m || (m = 1);
         window.START._int && clearInterval(window.START._int);
         window.START._int = setInterval(msg, 1000 * 60 * m);
         msg();
      }
   }])
